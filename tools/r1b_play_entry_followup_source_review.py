#!/usr/bin/env python3
"""Prepare the bounded second-pass source review for R1B fresh-player Play entry."""
...
SCHEMA = 1
PLAN_ID = "REVIEW-NET-R1B-PLAY-ENTRY-FOLLOWUP-26_2-001"
PREPARED_KIND = "r1b-play-entry-followup-source-review"
WORKSHEET_KIND = "r1b-play-entry-followup-source-review-worksheet"
COMMIT_POLICY = "EPHEMERAL_DO_NOT_COMMIT"
REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PLAN = REPO_ROOT / "vanilla/reviews/network/r1b-play-entry-followup-source-review-plan.json"
ROUTE_DISPOSITIONS = (
    "MANDATORY",
    "CONDITIONAL",
    "DEFAULT_EMPTY",
...
class FollowupReviewError(RuntimeError):
    """Fail-closed second-pass Play-entry source-review error."""


def _candidate(value: object, index: int) -> Candidate:
    if not isinstance(value, dict):
        raise FollowupReviewError(f"plan candidate {index} must be an object")
    required = {"var_id", "type_name", "method_name", "param_count", "review_focus"}
    allowed = required | {"exact_signature"}
    if not required <= set(value) or not set(value) <= allowed:
        raise FollowupReviewError(f"plan candidate {index} has unexpected or missing fields")
    ...
    if not isinstance(focus, list) or not focus or any(
        ...
        raise FollowupReviewError(f"plan candidate {index} review_focus must be non-empty strings")
    param_count = value["param_count"]
    if type(param_count) is not int or param_count < 0:
        raise FollowupReviewError(f"plan candidate {index} param_count must be non-negative integer")
    ...
    if exact_signature is not None and (
        ...
        raise FollowupReviewError(f"plan candidate {index} exact_signature must be non-empty")
    return Candidate(
...
def load_plan(path: Path = DEFAULT_PLAN) -> tuple[Candidate, ...]:
    try:
        ...
        raise FollowupReviewError(f"invalid follow-up review plan JSON: {error}") from error
    if not isinstance(value, dict) or set(value) != {"schema", "id", "candidates"}:
        raise FollowupReviewError("follow-up review plan has unexpected fields")
    if value["schema"] != SCHEMA or value["id"] != PLAN_ID:
        raise FollowupReviewError("follow-up review plan identity mismatch")
    raw = value["candidates"]
    if not isinstance(raw, list) or not raw:
        raise FollowupReviewError("follow-up review plan candidates must be non-empty")
    ...
    if len(ids) != len(set(ids)):
        raise FollowupReviewError("follow-up review plan contains duplicate candidate ids")
    if len(selectors) != len(set(selectors)):
        raise FollowupReviewError("follow-up review plan contains duplicate effective selectors")
    return candidates
...
def _external_fresh_dir(path: Path) -> Path:
    if path.exists() or path.is_symlink():
        raise FollowupReviewError(f"output directory must not already exist: {path}")
    ...
    try:
        ...
        return resolved
    raise FollowupReviewError(
        "Play-entry follow-up review contains official source text and must live outside the repository"
    )
...
def _resolve_all(conn: sqlite3.Connection) -> list[tuple[Candidate, sqlite3.Row]]:
    ...
    for candidate in CANDIDATES:
        ...
        except FollowupReviewError as error:
            failures.append(str(error))
    if failures:
        details = "\n".join(f"  - {failure}" for failure in failures)
        raise FollowupReviewError(f"selector preflight failed:\n{details}")
    return resolved
...
def _source_excerpt(archive: zipfile.ZipFile, row: sqlite3.Row) -> str:
    ...
    try:
        ...
        raise FollowupReviewError(f"source member missing: {path}") from error
    lines = text.splitlines()
    start, end = int(row["start_line"]), int(row["end_line"])
    if not (1 <= start <= end <= len(lines)):
        raise FollowupReviewError(f"invalid Atlas line range for {path}: {start}-{end}")
...
def prepare(output_dir: Path, db: Path, source: Path, lock: Path) -> dict[str, object]:
    ...
    try:
        ...
        manifest = {
            ...
            "next_required_step": (
                "Inspect all exact bodies and follow only remaining material delegates. "
                "Then construct the minimum final Play-entry VAR/SEM gate; this review is not admission."
            ),
        }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="r1b-play-entry-followup-source-review")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"play_entry_followup_review={args.output_dir}")
        ...
        print(f"R1B Play-entry follow-up source-review error: {error}", file=sys.stderr)
        return 2
