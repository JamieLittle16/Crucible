        ...
        if c in ('"', "'"):
            ...
            while i < n:
                ...
                if text[i] == quote:
                    ...
                    break
                i += 1
            out.append(Token("string" if quote == '"' else "char", text[start:i], start_line, start, i))
            continue
        ...
        if c.isdigit():
            ...
            while i < n and (text[i].isalnum() or text[i] in "._+-"):
                if text[i] in "+-" and text[i - 1] not in "eEpP":
                    break
                i += 1
            out.append(Token("number", text[start:i], line, start, i))
            continue
        matched = None
        for op in multi_ops:
...
def normalized_fingerprint(tokens: Sequence[Token]) -> str:
    """Fingerprint semantic Java tokens, ignoring whitespace/comments but preserving literals."""
    canonical = "\x1f".join(f"{token.kind}:{token.text}" for token in tokens)
        ...
        for typ in types:
            ...
            for method in methods:
                if method.body_open is not None and method.body_close is not None:
                    ...
                    body_raw = text[tokens[method.body_open].end:tokens[method.body_close].start].encode()
...
def render_markdown_report(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        f"**Generated by:** Vanilla Atlas {report['atlas_version']}",
        "",
        ...
        f"- fingerprint algorithm: `{report['fingerprint_algorithm']}`",
...
def cmd_status(db_path: Path) -> int:
    ...
    print(f"fingerprints {meta.get('fingerprint_algorithm', '?')}")
...
def cmd_record_template(db_path: Path, query: str, var_id: str) -> int:
    ...
    if len(rows) != 1:
        ...
        return 2
    row = rows[0]
    method_hashes = conn.execute("SELECT normalized_sha256,body_sha256 FROM methods WHERE id=?", (row['id'],)).fetchone()
    fp = method_hashes[0]
    body_fp = method_hashes[1]
    meta = dict(conn.execute("SELECT key,value FROM meta"))
    ...
    record = {
        ...
        "source": {
            ...
            "fingerprint_algorithm": meta.get("fingerprint_algorithm", FINGERPRINT_ALGORITHM),
...
def _load_review_records(records_dir: Path) -> list[tuple[Path, dict[str, object]]]:
    ...
    seen_sources: dict[tuple[str, str], tuple[str, Path]] = {}
    for path in sorted(records_dir.rglob("*.json")):
        ...
        if var_id in seen_ids:
            raise SystemExit(f"duplicate Vanilla Atlas record id {var_id}: {seen_ids[var_id]} and {path}")
        seen_ids[var_id] = path
        source = data.get("source")
        if isinstance(source, dict):
            key = (str(source.get("type", "")), str(source.get("signature", "")))
            if key != ("", "") and key in seen_sources:
                previous_id, previous_path = seen_sources[key]
                raise SystemExit(
                    f"source method claimed by multiple records: {previous_id} ({previous_path}) and {var_id} ({path})"
                )
            seen_sources[key] = (var_id, path)
...
def cmd_sync_records(db_path: Path, records_dir: Path) -> int:
    ...
    for path, record in records:
        ...
        expected_algorithm = str(source.get("fingerprint_algorithm", ""))
        expected_fp = str(source.get("normalized_sha256", ""))
        if not expected_algorithm or not expected_fp:
            print(f"record must pin fingerprint algorithm and normalized_sha256: {path}", file=sys.stderr)
            return 2
        var_id = str(record["id"])
        conn.execute("DELETE FROM classifications WHERE source='manual' AND reason=?", (var_id,))
        conn.execute("DELETE FROM semantic_edges WHERE var_id=?", (var_id,))
        row = conn.execute(
        ...
        if expected_algorithm != fingerprint_algorithm or expected_fp != actual_fp:
            status = "STALE"
            stale += 1
            reason = "fingerprint algorithm changed" if expected_algorithm != fingerprint_algorithm else "fingerprint changed"
            print(f"STALE {record['id']}: {reason}")
        existing = conn.execute("SELECT var_id FROM tracking WHERE method_id=?", (method_id,)).fetchone()
        if existing is not None and existing[0] not in (None, "", var_id):
            print(f"source method already claimed by VAR {existing[0]}: {qname}#{signature}", file=sys.stderr)
            return 2
        conn.execute(
            "UPDATE tracking SET review_status=?,var_id=?,notes=? WHERE method_id=?",
            (status, var_id, json.dumps(record.get("notes", []), separators=(",", ":")), method_id),
        )
...
def cmd_next(db_path: Path, frontier_name: str, config_path: Path | None, limit: int) -> int:
    ...
    for offset in range(0, len(ids), 700):
        ...
        for row in rows:
            ...
            status_rank = 0 if review_status == "STALE" else REVIEW_RANK.get(review_status, 0)
            if review_status in {"INTEGRATED", "PERFORMANCE_QUALIFIED"}:
                continue
            ...
            score = (
                ...
                - 8.0 * status_rank
                + (100.0 if review_status == "STALE" else 0.0)
