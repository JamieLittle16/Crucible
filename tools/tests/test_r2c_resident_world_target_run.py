...
from tools import r2c_resident_world_target_run as target_run


def raw_artifact(cpu: str = "3") -> dict[str, object]:
    return {
        "schema": 1,
        "benchmark": target_run.BENCHMARK,
        "mode": "full",
...
class R2CResidentWorldTargetRunTests(unittest.TestCase):
    ...
    def test_benchmark_command_is_full_locked_release_and_fail_closed_on_affinity(self) -> None:
        command = target_run.benchmark_command("7", Path("/tmp/raw.json"))
        self.assertEqual(command[:4], ["taskset", "-c", "7", "cargo"])
        self.assertIn("--release", command)
        self.assertIn("--locked", command)
        self.assertIn("resident_world_lifecycle_bench", command)
    ...
    def test_annotation_stamps_explicit_non_hosted_target_witness(self) -> None:
        ...
        self.assertEqual(annotated["hardware"], source["hardware"])
    ...
    def test_annotation_rejects_mismatched_observed_affinity(self) -> None:
        with self.assertRaisesRegex(target_run.TargetRunError, "did not remain pinned"):
            target_run.annotate_artifact(raw_artifact("4"), "3")

    def test_annotation_rejects_wrong_mode_or_diagnostic_policy(self) -> None:
        ...
        wrong_policy["hosted_ci_is_diagnostic_only"] = False
        with self.assertRaisesRegex(target_run.TargetRunError, "hosted_ci_is_diagnostic_only"):
            target_run.annotate_artifact(wrong_policy, "3")

    def test_annotation_refuses_preannotated_artifact(self) -> None:
        source = raw_artifact()
    ...
    def test_target_process_refuses_to_overwrite_existing_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "target-run.json"
    ...
    def test_target_process_rejects_failed_or_missing_benchmark_output(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "target-run.json"
