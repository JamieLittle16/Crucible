...
impl DeflateChunkPayloadDecoder {
    ...
    fn ensure_output(&mut self, required: usize) -> Result<(), CompressedPayloadError> {
        ...
        self.output.try_reserve_exact(additional).map_err(|_| {
            CompressedPayloadError::OutputBufferAllocationFailed {
                requested: required,
            }
        })?;
    ...
    fn inflate_exact<'a>(
        ...
        let output = self.output.get_mut(..max_decompressed_bytes).ok_or(
            CompressedPayloadError::OutputExceedsLimit {
    ...
    fn decode<'a>(
        ...
        match compression {
            ...
            ChunkCompression::Lz4 => {
                Err(CompressedPayloadError::CompressionNotAdmitted { compression })
            }
        }
    }
}
...
mod tests {
    ...
    const ZLIB: &[u8] = &[
        0x78, 0x9c, 0xf3, 0x48, 0xcd, 0x29, 0x4b, 0x55, 0x48, 0xca, 0x2f, 0xcd, 0x4b, 0x49, 0x4d,
        0x51, 0x48, 0xce, 0x4f, 0x49, 0x4d, 0x56, 0x48, 0xcb, 0xac, 0x28, 0x29, 0x2d, 0x4a, 0x05,
        0x00, 0x8c, 0x19, 0x0a, 0x3b,
    ];
    const GZIP: &[u8] = &[
        0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xf3, 0x48, 0xcd, 0x29, 0x4b,
        0x55, 0x48, 0xca, 0x2f, 0xcd, 0x4b, 0x49, 0x4d, 0x51, 0x48, 0xce, 0x4f, 0x49, 0x4d, 0x56,
        0x48, 0xcb, 0xac, 0x28, 0x29, 0x2d, 0x4a, 0x05, 0x00, 0xe5, 0x69, 0x81, 0x33, 0x1b, 0x00,
        0x00, 0x00,
    ];
