...
mod tests {
    ...
    fn first_selected_teleport_matches_exact_r1x_golden_payload() {
        let expected: &[u8] = &[
            0x01, 0x40, 0x24, 0xc8, 0x2a, 0xe0, 0x8d, 0x66, 0xf5, 0x40, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0xc0, 0x17, 0x42, 0xb9, 0x40, 0xa5, 0xe1, 0x97, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0xc2, 0xee, 0x33, 0x35, 0x41, 0x90, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
        ];
        let mut writer = PacketWriter::new(expected.len()).expect("exact teleport payload bound");
        CAPTURED.encode(&mut writer).expect("captured teleport fits");
    ...
    fn wrong_or_stale_ack_never_destroys_current_transaction() {
        ...
        assert_eq!(state.awaiting().expect("second remains pending").id, second.id);
        assert_eq!(state.acknowledge(99), TeleportAckResult::IdMismatch { expected: 2, received: 99 });
